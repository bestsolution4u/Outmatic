...
import 'package:outmatic/widget/item/batch_project_item.dart';

import '../../../app_bloc.dart';

class BatchScanPRDScreen extends StatefulWidget {
  @override
  _BatchScanPRDScreenState createState() => _BatchScanPRDScreenState();
}

class _BatchScanPRDScreenState extends State<BatchScanPRDScreen> {

  BatchBloc _batchBloc;
  TextEditingController _searchQueryController = TextEditingController();
  bool _isSearching = false;
  String searchQuery = "";
  List<BatchProjectModel> projects = [];
  ...
  Widget build(BuildContext context) {
    return BlocListener<BatchBloc, BatchState>(
      listener: (context, state) {
        if (state is BatchPRDFetchedState) {
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: _isSearching ? _buildSearchField() : Text("Zoek Project", style: TextStyle(
            fontSize: 20,
            color: Colors.white,
                ...
                Expanded(
                  child: Container(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) => BatchProjectItemWidget(project: projects[index], onPress: () {
  ...
  Widget _buildSearchField() {
    return TextField(
      ...
      onSubmitted: (value) {
        if (value.isNotEmpty) {
          _batchBloc.add(BatchPrdSearchEvent(query: value));
        }
      },
    );
  }
